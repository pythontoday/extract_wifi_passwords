...
def profile_list():
    ...
    all_profiles = subprocess.run('netsh wlan show profiles', stdout=subprocess.PIPE, encoding='utf-8')
...
def get_password(profiles):
    ...
    for profile in profiles: 
        password_raw = subprocess.run(f'netsh wlan show profile "{profile}" key=clear', stdout=subprocess.PIPE, encoding='utf-8')
